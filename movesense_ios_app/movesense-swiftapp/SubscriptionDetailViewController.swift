...
class SubscriptionDetailViewController : MovesenseViewController, UINavigationControllerDelegate
{
    ...
    override func viewDidLoad() {
        setupHandlers()
        debugPrint("fdkljsklfdklafjkdlajfl")
        super.viewDidLoad()
    }
...
class Vector3DSubscriptionHandler : BaseSubscriptionHandler {
    public var values = Array<(time: UInt32, vector:Vector3D)>()
    public var sampleRate = Int32(13)
    public var dataArrayName: String
    public var configPath: String
    public var newSamples: (Array<Vector3D>) -> () = { (vectors) in }
    ...
    public func handleData(_ response: MovesenseResponse) {
    ...
    public func appendData(_ data: DataVectors) {
...
class AccelerationHandler : Vector3DSubscriptionHandler {
    ...
    public override func handleData(_ response: MovesenseResponse) {
        let data = response.getVectors(self.dataArrayName)
        if data.time > 0 && !data.vectors.isEmpty {
            appendData(data)
            ...
            if self.viewController.fileOutputSwitch.isOn {
                for item in data.vectors {
                    self.dataFile?.write(String(format: "%f,%f,%f", item.x, item.y, item.z))
                    //debugPrint("ACC: " + String(format: "%f,%f,%f", item.x, item.y, item.z))
                    //debugPrint(self.viewController.uuid!)
                    
                    var fight = false
                    var direction = 0 // -1: left; 1: right
                    if (item.x * item.x + item.y + item.y + item.z + item.z > 300) {
                        fight = true
                    }
                    ...
                    for ip in ["192.168.137.1"] {
                        var url = URL(string: "http://" + ip + ":8000/upload/")!
                        var request = URLRequest(url: url)
                        let json: [String: Any] = ["x": item.x, "y": item.y, "z": item.z, "id": String(describing: self.viewController.uuid!)]
                        ...
                        if (fight && self.viewController.uuid == UUID(uuidString: "113D4184-9A41-55AF-15A1-990FC66FD6B2")) {
                            debugPrint("FIGHT!")
                            url = URL(string: "http://" + ip + ":8000/fight/")!
                        ...
                        if ((direction != 0) && self.viewController.uuid == UUID(uuidString: "5A233EEF-9C58-4922-A159-EB0EAE757ECF")) {
                            if (direction == -1) {
                                debugPrint("LEFT!")
                            } else {
                                debugPrint("RIGHT!")
                            }
                            let key = (direction == -1) ? "left" : "right"
                            url = URL(string: "http://" + ip + ":8000/" + key + "/")!
                            request = URLRequest(url: url)
                            request.httpMethod = "POST"
                            let task = URLSession.shared.dataTask(with: request) { data, response, error in
                                guard let data = data, error == nil else {                                                 // check for fundamental networking error
    ...
    public override func handleData(_ response: MovesenseResponse) {
        ...
        if data.time > 0 && !data.vectors.isEmpty {
            ...
            if self.viewController.fileOutputSwitch.isOn {
                for item in data.vectors {
                    self.dataFile?.write(String(format: "%f,%f,%f", item.x, item.y, item.z))
                    //debugPrint("GYR: " + String(format: "%f,%f,%f", item.x, item.y, item.z))
                    
                    for ip in ["192.168.137.1"] {
                        let url = URL(string: "http://" + ip + ":8000/upload_g/")!
                        var request = URLRequest(url: url)
                        let json: [String: Any] = ["x": item.x, "y": item.y, "z": item.z, "id": String(describing: self.viewController.uuid!)]
